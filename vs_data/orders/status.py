...
def update_link_db_orders_data(fmlinkdb, update_response):
    ...
    for order in wc_updated_orders:
        completion_date = order['date_completed'].replace("T", " ")
        sql = (
...
def update_packed_orders_status(fmlinkdb, wcapi, cli: bool=False, status="completed"):
    ...
    # orders = [{'link_wc_order_id': 46015, 'full_name': 'Caron Elgey', 'status': 'packing', 'selected': 'Yes'}]
    log.debug(orders)

    if orders:
        wc_response = wc_orders_update_status(wcapi, orders, status=status)
        log.info("WooCommerce updated")
        link_result = update_link_db_orders_data(fmlinkdb, wc_response)
